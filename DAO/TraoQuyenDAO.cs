...
namespace DOAAn.DAO
{  /*Nếu muốn trao quyền hay tước quyền thì cứ tạo hàm ròi làm y chang trong này nhưng nhớ thay tên procedure và tên người dùng*/
    public class TraoQuyenDAO
    {
        ...
        public static TraoQuyenDAO Instance
        {
            get
            {
                if (instance == null) { instance = new TraoQuyenDAO(); }
                return instance;
            }
            set { instance = value; }
        }
        ...
        public void SinhVienXemTHongTIN(int i)
        {
            if (i == 0) {
               DataProvider.Instance.ExecuteNonQueryNoResult("USE PhanQuyen  ; REVOKE EXECUTE ON  XemThongTinCaNhan from  SinhVien;");
                ...
                /*XemThongTinCaNhan là tên proc, SinhVien là user, này t code sẵn hết ròi, m chỉ việc copy paste thoi, trong trường hơp m phân quyền giáo viên bằng sql server*/
            }
            else if(i==1)
            {
